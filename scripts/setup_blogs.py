...
def insert_blogs():
    ...
    # Ensure directory exists if we ever change paths, but for now just check existence
    if not os.path.exists(db_path):
        print(f"Warning: {db_path} not found. A new database will be created, but existing data might be lost if you expected an existing one.")
