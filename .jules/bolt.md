...
## 2026-09-24 - Target Specific Progress Query\n**Learning:** When fetching `course_progress` data for the student dashboard, it's a common oversight to query all progress rows for a `user_id` globally. In a platform where a user may enroll in multiple courses over time, fetching progress globally pulls unnecessary rows into application memory. By simply filtering on `course_id = ?` as well, we significantly reduce data transfer and memory footprint, solving a hidden N+1-style data overfetching problem.\n**Action:** Whenever retrieving data related to a specific entity (like a single course view), always ensure database queries explicitly filter by that entity's ID, even if filtering just by `user_id` technically works. Limit returned datasets early in SQL rather than discarding in Python.
